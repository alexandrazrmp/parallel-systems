...
# average over processes and sparsity for clarity
sub = df_avg.groupby("iterations")[["Total_serial_CSR","Total_parallel_CSR","Serial_dense","Parallel_dense"]].mean()
...
# ------------------------------
plt.figure(figsize=(8,6))
# average over processes and iterations for clarity
sub = df_avg.groupby("sparsity")[["Total_serial_CSR","Total_parallel_CSR","Serial_dense","Parallel_dense"]].mean()
sub.plot(marker='o', ax=plt.gca())
plt.xlabel("Sparsity (%)")
plt.ylabel("Time (s)")
...
# ------------------------------
plt.figure(figsize=(8,6))
# average over processes, sparsity, and iterations
sub = df_avg.groupby("N")[["Total_serial_CSR","Total_parallel_CSR","Serial_dense","Parallel_dense"]].mean()
sub.plot(marker='o', ax=plt.gca())
...
# ------------------------------
# 4️⃣ Parallel scaling breakdown
# ------------------------------
# CSR breakdown: Construction, Message Send, Multiply, Total
plt.figure(figsize=(8,6))
df_csr = df_avg.groupby("processes")[["CSR_construction","CSR_msg_send","CSR_parallel","Total_parallel_CSR"]].mean()
...
# Dense breakdown: Message Send, Multiply, Total
plt.figure(figsize=(8,6))
df_dense = df_avg.groupby("processes")[["dense_message_send_time","dense_mult_time","Parallel_dense"]].mean()
...
print("✅ All 4 main plots saved in results/plots/")
